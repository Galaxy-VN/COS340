...
if (isset($url[0]) && $url[0] === "api") {
    ...
    switch ($resource) {
        case "product":
            if ($method === "GET") {
                if ($id === "category" && isset($url[3])) {
                    $controller->category($url[3]);
                } elseif ($id) {
                    $controller->show($id);
                } else {
                    $controller->index();
...
// Web Routing
$routeController =
    isset($url[0]) && $url[0] != "" ? ucfirst(strtolower($url[0])) : "Default";
...
if (in_array($controllerName, $adminControllers)) {
    ...
    isset($adminActions[$controllerName]) &&
    in_array($action, $adminActions[$controllerName])
) {
    $requiresAdmin = true;
}
...
if (
    isset($loginActions[$controllerName]) &&
    in_array($action, $loginActions[$controllerName])
) {
    $requiresLogin = true;
}
